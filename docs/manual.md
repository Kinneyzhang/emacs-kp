...
[Installation and first example](../README.md).
...
Measured on the bundled sample texts (`benchmarks/ekp-bench.el`), batch
Emacs 30.2, Apple Silicon; see docs/architecture.md for methodology:
...
Run `make check`, native parity/fuzz checks and dictionary provenance checks before a release. Keep Elisp and C output equivalent; update both ABI versions when the interface changes. Record user-visible changes in `CHANGELOG.md`. Build from the reviewed tag and publish immutable artifacts with SHA-256 checksums. Never move a published tag or replace its assets.
