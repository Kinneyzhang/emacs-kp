...
基于内置示例文本(`benchmarks/ekp-bench.el`)、batch Emacs 30.2、Apple
Silicon 测得;方法见 docs/architecture.zh-CN.md:
...
发布前运行 `make check`、原生实现对齐与 fuzz 检查，以及词典来源检查。Elisp 与 C 输出必须等价；接口变化时同步更新 ABI 版本。在 `CHANGELOG.md` 记录用户可见变化，从审核后的 tag 构建并发布带 SHA-256 校验值的不可变产物，不移动已发布 tag 或替换其产物。
